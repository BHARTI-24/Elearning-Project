...
class users
{
	...
	public function login($email,$pass)                  // funtion for login(called in login_submit.php)
	{
		...
		$query=$this->conn->query("select email,password from login_site where email='$email' and password='$pass'");
	...
	{
		$query=$this->conn->query("select * from login_site where email='$email'");
